...
TreeEntry BinarySearchTree<TreeEntry>::secondMinimum()
{
	...
	//pos-condicao: caso tenham mais que dois elementos retorna o segundo menor elemento da arvore, caso nao tenha uma mensagem de erro aparecera;
	
	//Analise Assintotica: no pior caso do metodo esse tera que percorrer a arvore em busca do segundo menor elemento passando por log2 N elementos sendo entao O(log2 N) ou O(height);
	
	TreePointer aux = root; // Auxiliar que irah percorrer a arvore ateh o menor elemento;
	TreePointer sm; // Second Minimum que irah percorrer junto ao auxiliar porem um noh acima;
	
	if(aux == NULL){ // Caso a arvore esteja vazia;
		cout << "Arvore Vazia!" << endl;
	}else{
		if(aux->leftNode == NULL && aux->rightNode == NULL){ //Caso a arvore tenha um unico elemento;
			cout << "Arvore com um unico elemento, portanto sem segundo minimo" << endl;
			return INT_MAX;
		}else{
		
			while(aux->leftNode != NULL){ //Percorrer ateh o menor numero da arvore;
				sm = aux;
				aux = aux->leftNode;
			}
			
			if(aux->rightNode == NULL){ //Caso o menor elemento da arvore nao tenha ramos para direita dele o noh pai do minimo sempre serah o second minimum;
				return sm->entry;
			}else{
				sm = aux->rightNode;
				if(sm->leftNode == NULL){ //Quando houver um noh a direita do minimo, verifica se deste noh hah valores para esquerda;
					return sm->entry;
				}else{
					while(sm->leftNode != NULL){ //Quando houver um noh a direita do minimo, a partir deste ele ira percorrer o maximo para esquerda;
						sm = sm->leftNode;	
					}
					return sm->entry;
				}
